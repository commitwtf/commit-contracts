...
contract EASVerifier is IVerifier {
    ...
    function verify(address participant, bytes calldata data, bytes calldata) external view override returns (bool) {
        (bytes32 schemaUID, address attester) = abi.decode(data, (bytes32, address));
        // Return true if the account has a valid attestation
        return easIndexer.getSchemaAttesterRecipientAttestationUIDCount(schemaUID, attester, participant) > 0;
    }
}
